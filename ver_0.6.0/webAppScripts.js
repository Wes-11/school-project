...
function deleteCookie(name) {
  document.cookie = name + '=;expires=Sat, 8 Dec 2018 00:00:01 GMT;';
  //log out just deletes the cookie
}
function buildTable(beer) {
...
function buildFavs(beer) {
  ...
  //edit, dont do that. It actaully made it harder to read
  return row;
}

function logCheck() {
  var exists = document.cookie.match('(^|;)\\s*' + 'username' + '\\s*=\\s*([^;]+)');
  if (exists == null) {
...
function childSpawn(table) {
  ...
  name = getUser(username);
    ...
    return '<table cellpadding="0" cellspacing="0" border="0" style="padding-left:50px;">' +
      ...
      '</tr>' +
    '</table>';
    //do not apply automated formating to this, that actually broke the markup
  ...
  function tabulate(evt, table) {
    ...
    for (i = 0; i < content.length; i++) {
      ...
      //edit, thats because it just looks like a for each loop when in reality it behaves like a normal loop
